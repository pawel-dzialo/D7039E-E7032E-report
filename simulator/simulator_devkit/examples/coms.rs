                ...
                Ok(count) if count > 0 => {
                   // rprintln!("RX");
                    ...
                    for c in buf[0..count].iter_mut() {
                       // rprintln!("{:b}",*c);
                        cx.shared.usb_buffer.lock(|usb_buffer|{
                            usb_buffer[*cx.local.usb_index][i] = *c;
                        });
                        if(i==0){
                            i+=1;
                        }
                    ...
                    if(*cx.local.usb_index<7){
                        *cx.local.usb_index+=1;
                    }
                    else{
                       // rprintln!("sending");
                  ...
                  //      cx.shared.usb_buffer.lock(|usb_buffer|{
                    ...
                    /*     match serial.write(&[0xFFu8,0xFFu8]) {
                            Ok(len) if len > 0 => {
                                //(cx.local.received) += 1;
                               // rprintln!("received {} bytes, errors{}, data{:?}", *(cx.local.received)*64,*(cx.local.errors), &buf[write_offset..count]);
                                //write_offset += len;
                            }
                            Err(_err) => {
                                //(cx.local.errors)+=1;
                                //rprintln!("received {} bytes, errors{}, data null", *(cx.local.received)*64,*(cx.local.errors));
                            }
                            _ => {}
                        } */
                    //}
...
//
//    There is no usb_fs task in rtic_usb_serial, so not sure what to say about that. I guess the
//    operations that task performs here are moved to idle and busy polled there.
//    EP_MEMORY is a local resource in the init function in both cases and is assigned to the usb bus
//    on instantiation in both cases. 
//    i guess the difference between this and the example is here we use interrupts while
//    the example is constantly busy polling.
//
